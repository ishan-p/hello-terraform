...
resource "aws_lb_target_group_attachment" "targets_app1_target1" {
  ...
  target_id        = var.app_private_instance1_subnet1_id
  port             = 80
}

resource "aws_lb_target_group_attachment" "targets_app1_target2" {
  target_group_arn = aws_lb_target_group.target_group_app1.arn
  target_id        = var.app_private_instance2_subnet1_id
  port             = 80
}

resource "aws_lb_target_group_attachment" "targets_app1_target3" {
  target_group_arn = aws_lb_target_group.target_group_app1.arn
  target_id        = var.app_private_instance1_subnet2_id
  port             = 80
}
